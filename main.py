...
def send():
    ...
    text_box.create_text(window.winfo_width()*.9,window.winfo_height()*.12,text="You: ", anchor=tk.E, fill=FONT_COLOR)
    text_box.create_text(window.winfo_width()*.9,window.winfo_height()*.14,text=sent, width=(window.winfo_width())*2/3,  anchor=tk.NE, fill=FONT_COLOR)
...
def respond():
    ...
    text_box.create_text(window.winfo_width() * .1, window.winfo_height() / 2.4, text="boyidiot's Neural Network: ",
                         width=(window.winfo_width()) * 2 / 3, anchor=tk.W,fill=FONT_COLOR)
    text_box.create_text(window.winfo_width() * .1, window.winfo_height() / 2.2, text=response,
...
input = tk.Entry(master=labelBorder,width = INPUT_WIDTH,highlightthickness=2,highlightcolor=DARK_ACCENT_COLOR, bd=0)
scrollable.pack()
inputLabel.pack()
labelBorder.pack()
...
text_box.create_text(WIDTH/2,HEIGHT/2.5,text="Welcome to boyidiot's totally advanced AI chatbot that will tell you jokes based on what you tell it! Let's get started, hello!", anchor=tk.N, width=WIDTH*(2/3),fill=FONT_COLOR)
text_box.pack(fill=tk.BOTH)
window.mainloop()